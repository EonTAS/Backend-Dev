...
def login():
    ...
    if request.method == "POST":
        ...
        #if entry doesnt exist or password has doesnt match, then redirect to login page again
...
def get_user(username):
    ...
    if request.method == "POST": #if post request then user wanting to deposit money
...
def delete_account(username):
    #only allow deleting if logged in user is admin or if they are the user trying to delete themself
...
def editItem(id):
    if session.get("user", "") == "admin": #only allow edit if admin
        item = request.form.to_dict() #request form already follows correct format for data in database, so get that into dict
        item["sold"] = bool(item["boughtBy"])
        mongo.db.stock.update_one({"_id": ObjectId(id)}, {"$set": item})
...
def add_basket(product_id):
    if product_id not in session["basket"]:
        ...
        session.modified = True #mark session as modified so that it saves edit
...
def remove_basket(product_id):
    if product_id in session["basket"]:
        session["basket"].remove(product_id) #remove item from session cookies
        session.modified = True #mark session as modified so that it saves edit
